 ...
 */
public class Memo implements ByteSerializable, JsonSerializable, Parcelable {
...
//            byte[] nonceBytes = Util.revertBytes(nonce.toByteArray());
    ...
    protected Memo(Parcel in) {
        try {
            this.from = new Address(in.readString());
            this.to = new Address(in.readString());
            this.nonce = (BigInteger) in.readSerializable();
            this.message = in.createByteArray();
            this.plaintextMessage = in.readString();
        } catch (MalformedAddressException e) {
            e.printStackTrace();
        }
